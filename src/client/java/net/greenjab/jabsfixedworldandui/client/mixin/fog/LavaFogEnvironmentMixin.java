...
public abstract class LavaFogEnvironmentMixin {
    ...
    private float lessLavaFog(float constant, @Local(argsOnly = true) Camera camera) {
        int i = 0;
        if (camera.entity() instanceof Player entity) {
            for (ItemStack item : JabsFixedWorldAndUI.getArmor(entity)) {
                i += JabsFixedWorldAndUI.enchantLevel(item, "fire_protection");
            }
        }
